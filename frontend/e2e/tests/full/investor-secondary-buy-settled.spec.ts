...
test.describe('Investor secondary buy until settled', () => {
  test.skip(!gatewayUrl, 'Set E2E_GATEWAY_URL to run against a live gateway');

  test('investor buy order on secondary listing reaches SETTLED status', async ({ page }) => {
    ...
    test.skip((await listingLink.count()) === 0, 'No secondary listings — sell tokens or seed first');
