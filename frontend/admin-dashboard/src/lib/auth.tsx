...
import type { TokenResponse, UserProfile, UserRole } from '@/types/api';

const STORAGE_KEY = 'tokenrealty.admin.auth';
const REFRESH_CHECK_MS = 30_000;
const ADMIN_ROLES: UserRole[] = ['ADMIN', 'COMPLIANCE', 'PROPERTY_MANAGER'];
...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const login = useCallback(async (email: string, password: string) => {
    ...
    if (!ADMIN_ROLES.includes(tokens.role)) {
      throw new Error('Admin, Compliance, or Property Manager role required');
    }
    applyTokens(tokens);
    setIsLoading(true);
  }, [applyTokens]);
  ...
  const value = useMemo(
    ...
    [user, stored?.accessToken, isAdmin, isLoading, login, logout],
  );
